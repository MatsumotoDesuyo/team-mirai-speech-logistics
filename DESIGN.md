...
- Stage 4 後の Doc スリム化 + 章構成変更（2026-05-31）— ミッションオーナーのレビューで「演説場所選定メンバーが知る必要のない内容」を §12（公共施設等・文書図画掲示・公道毀損防止・期間内外文言制約）・§13（入試 URL テーブル）・§14（Apps Script・進捗管理シートテンプレ構造）から削除。chief-of-staff プロンプト本文を MANUAL.md §14 内ではなく **Google Docs の別タブ** に分離する配置に変更（プロンプトは Gemini 経路の一般サポーターも使うため、エンジニア向け章の内側に置くのは適切でなかった）。Markdown 側では引き続き `prompts/chief-of-staff.md` に分離保持。プロンプト内の §12.5 参照は §5.1 / §6.4 + 期間外文言禁止の直接記述に置換。
