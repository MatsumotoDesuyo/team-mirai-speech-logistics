...
- **告知文作成**: 文体は GUIDELINES.md §3 準拠（受動態回避、二重否定禁止、曖昧副詞回避、1 文 50 字推奨）。選挙期間外と期間中で許される文言が異なる（§5.1 戸別訪問・期間制約、§6.4 候補者届け出日との関係参照。**期間外は「投票してください」「立候補予定」等の選挙運動文言を禁止、期間中は解禁**）。**LINE オプチャアンケートを作成する際は §10.3.2 で定めた 4 選択肢（「手伝いに行きます！」「行けたら行きます！」「気持ちだけ応援！」「場所取りします！」）を必ず使用する**。独自の質問形式（時間帯選択フォーム等）を勝手に作らない。
