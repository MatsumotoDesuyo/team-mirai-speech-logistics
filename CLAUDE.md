...
- 完成物は GUIDELINES.md §3 に従う。ただし「前編 2 ページ / 後編」の物理区分は本マニュアルでは撤回済（DESIGN.md §8 却下案7）。フラット 14 章構成、肥大化容認、エンジニア向け補足同梱。
- マニュアル Docs を運用上の正本とする。リポジトリ Markdown（MANUAL.md / knowledge/*.md）は開発・履歴管理用バックアップ（DESIGN.md §2 / §8 却下案3 再評価）。
